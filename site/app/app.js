...
async function exam(aid){
  ...
  let timerHandle=null,closed=false,lastViolationAt=0,violations=0;
  ...
  const finishToResults=()=>{cleanup();state.view='student-results';render()};
  ...
  const onVisibility=()=>{if(document.hidden)report('visibility_hidden','You left the exam tab.')};
  const onBlur=()=>report('window_blur','The exam window lost focus.');
  ...
  const draw=()=>{
    E.innerHTML=
      ...
      '<div class="top exam-top"><div><h2 style="margin:0">'+h(d.exam.title)+'</h2><div class="mut">'+h(d.exam.instructions||'')+'</div><div class="integrity-status">Secure exam · Violations: <b id="violation-count">'+violations+'</b>/3</div></div><div><div class="timer" id="tm"></div><button id="fsbtn" class="btn alt sm" type="button">Fullscreen</button></div></div>'+
    ...
    document.querySelectorAll('[data-q]').forEach(el=>el.onchange=async()=>{
      const q=el.dataset.q;
      let val;
      if(el.type==='checkbox')val=[...document.querySelectorAll('[data-q="'+q+'"]:checked')].map(x=>x.value);
      else val=el.value;
      await api('/attempts/'+aid,{method:'PUT',body:JSON.stringify({question_id:Number(q),answer:val})});
    });
  ...
  const tick=()=>{
    ...
    if(remaining<=0){
      ...
      api('/attempts/'+aid+'/submit',{method:'POST',body:'{}'}).finally(()=>{document.title='Redbridge Exam Platform';finishToResults()});
    }
  };
